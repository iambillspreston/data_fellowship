...
inspect(sad_rules[1:20])

```

```{r}
sad_rules2 = DATAFRAME(sad_rules)
#sadder_rules = subset(sad_rules, subset=!(rhs %in% 'excited')


sad_rules2[sad_rules2['LHS'] == "{stress}", ]
```

```{r}

happy_rules = sort(
  subset(rules, subset=lhs %in% 'happiness'),
  by = 'lift',decreasing = T
)
happy_rules2 = DATAFRAME(happy_rules)
#sadder_rules = subset(sad_rules, subset=!(rhs %in% 'excited')


happy_rules2[happy_rules2['LHS'] == "{happiness}", ]
